...
#ifndef FIREFLY_SAMPLE_THRESH
#define FIREFLY_SAMPLE_THRESH 5.0
#endif
...
{   
    ...
    float currLum = luminance(currData.rgb);
    float currLogLum = log2(currLum + 1e-6);
    ...
    {   
        ...
        if (clamp(sampleCoord, ivec2(0), ivec2(internalScreenSize) - 1) == sampleCoord) {
            ...
            if (!any(isnan(sampleData))) {
                float sampleLum = luminance(sampleData.rgb);
                if (sampleLum > currLum * FIREFLY_SAMPLE_THRESH) {
                    continue;
                }
                ...
                float sampleWeight = exp(-varianceWeight * (
                    ...
                    + intensityWeight * min(abs(log2(sampleLum + 1e-6) - currLogLum), 3.0)
                    )
                );
    ...
    vec4 result;
    if (weights > exp2(-16.0)) {
        result = vec4(samples / weights);
    } else {
        result = currData;
    }

    // --- ЕДИНСТВЕННАЯ ДОБАВЛЕННАЯ ХУЙНЯ: просто режем яркость, если пиздец как ярко ---
    float resultLum = luminance(result.rgb);
    if (resultLum > 10000.0) {
        result.rgb *= 10000.0 / resultLum;
    }
